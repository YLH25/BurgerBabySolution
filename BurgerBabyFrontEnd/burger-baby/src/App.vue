  ...
  <div>
    <nav class="navbar navbar-expand-md navbar-warning fixed-top bg-warning">
      <router-link class="navbar-brand" to="/">
        <img
          id="food-icon"
          :style="{ width: '4em', height: '' }"
          src="../src//assets/foodicon.png"
          alt=""
        />
      </router-link>
      ...
      >
        ...
        <div class="memberInfo me-5 p-1">
          <div v-if="$store.state.isLoggedIn" class="d-flex">
            <span class="me-5 fs-1">{{
              "Hi," + $store.state.memberInfo.name
            }}</span>
            <span class="d-flex align-items-center justufy-content-center"
              ><button type="button" @click="logout" class="btn btn-success">
                會員登出
              </button></span
            >
          </div>
          <div v-else class="d-flex align-items-center justufy-content-center">
            <span class="d-flex me-5">
              <div class="d-flex" style="flex-direction: column">
                <div class="p-1">
                  <label style="width: 4.5em" for="email">電子郵件:</label>
                  <input
                    v-model="email"
                    id="email"
                    type="email"
                ...
                <div class="p-1">
                  <label style="width: 4.5em" for="password">密碼:</label>
                  <input
                    v-model="password"
                    id="password"
                    type="password"
...
export default {
  ...
  methods: {
    ...
    initMember() {
      this.$store.dispatch("getAccessToken").then((accessTokenIsGottern) => {
        if (accessTokenIsGottern) {
          this.$store.dispatch("getMember").then((memberIsGottern) => {
            if (memberIsGottern) {
              this.$store.commit("changeLoginStatus", true);
            } else {
              this.$store.commit("changeLoginStatus", false);
              this.$store.commit("changeAccessToken", "");
              console.log("獲取會員資料失敗，請重新登入");
            }
          });
        } else {
          this.$store.commit("changeLoginStatus", false);
          this.$store.commit("changeAccessToken", "");
          console.log("獲取通行失敗，請重新登入");
        }
      });
    },
    login() {
